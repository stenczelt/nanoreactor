...
class Nanoreactor(Molecule):
    def __init__(self, xyzin=None, qsin=None, properties='properties.txt', dt_fs=0.0, boin='bond_order.list',
                 bothre=0.0,
                 enhance=1.4, mindist=1.0, printlvl=0, known=['all'], exclude=[], learntime=100.0, cutoff=100.0,
        ...
        self.IsoLabels = []
        if self.boHave:
            self.boFiltered = self.timing(self.tsFilter, "Filtering bond order time series", self.boSparse, self.boThre,
                                          self.freqCutoff, 'bo', 'plot_bo.pdf' if plot else None)
        else:
            ...
            self.dxSparse, self.dxThre = self.timing(self.measureDistances, "Measuring interatomic distances",
    ...
    def makeGlobalGraphs(self, tsData, tsThre, mode):
        ...
        for i in range(len(self)):
            ...
            if i > 0 and self.printlvl >= 2:
                added = ['%i-%i' % (tsPairs[i, 0] + 1, tsPairs[i, 1] + 1) for i in
                         sorted(list(set(gg) - set(lastGraph)))]
                removed = ['%i-%i' % (tsPairs[i, 0] + 1, tsPairs[i, 1] + 1) for i in
                           sorted(list(set(lastGraph) - set(gg)))]
    ...
    def makeMoleculeGraphs(self):
        ...
        for igg, globalGraph in enumerate(self.global_graphs):
            ...
            for G in MolGphs:
                ...
                if (ef in self.KnownFormulas or wildmatch(ef, self.KnownFormulas) or (
                        igg == 0 and 'ALL' in [i.upper() for i in self.KnownFormulas])):
                    known_iidx.add(iidx)
        ...
        if self.printlvl >= 2:
            for molID in TimeSeries:
                ...
                ef_str = ts['graph'].ef()
                ts_str = ''.join(
                    [("/\\u203E%i\\u203E\\" % t[0]) if t[1] else ('_%i_' % t[0]) for t in encode(ts['raw_signal'])])
                print(("molecule index %i formula %s iidx %s atoms %s series %s" % (
                    MolIDs.index(molID), ef_str, iidx_str, atom_str, ts_str)).encode('utf-8'))
    ...
    def allStable(self, frame, atoms, direction):
        ...
        stable_mol_atoms = sorted(
            list(itertools.chain(*[self.TimeSeries[self.MolIDs[i]]['graph'].L() for i in stable_midx])))
    ...
    def findReactionEvents(self):
        ...
        if self.printlvl >= 2:
            ...
            for iev, evid in enumerate(EventIDs):
                Event = Events[evid]
                print("Reaction event found (%i/%i) : frames %i-%i : %s" % (
                    iev + 1, len(Events), Event['frames'][0], Event['frames'][1], Event['equation']))
    ...
    def analyzeIsomers(self):
        ...
        for iidx, G in enumerate(self.Isomers):
            ...
            IsomerData[iidx] = OrderedDict(
                [('graph', G), ('midx', []), ('firstFound', len(self)), ('stableIndices', []),
        ...
        if self.printlvl >= 0:
            ...
            print("%10s %20s %10s %10s %10s %10s %10s %10s %20s" % (
                "Index", "Formula", "Instances", "firstFound", "firstStabl", "maxStable", "meanStable", "Flag",
                "Color"))
            print("=" * 120)
        ColorNum = 0
        nsave = 0
        for key in sorted(sortKeys):
            ...
            if self.printlvl >= 0:
                ...
                print("%10i %20s %10i %10i %10i %10i %10.2f %10s %20s" % (
                    iidx, IData['graph'].ef(), len(IData['midx']), IData['firstFound'], firstStable, maxInterval,
        ...
        if self.save_molecules:
            for key in sorted(sortKeys):
                ...
                if IData['flag'] == "Found":
                    ...
                    if self.printlvl >= 2:
                        print("Writing iidx %i (%i/%i stable), frames %i-%i to file %s : %s" % (
                            iidx, nsave + 1, nFound, frame, frame + intvl, fout, formula))
                    elif self.printlvl >= 1:
                        print("Writing isomer %i/%i, frames %i-%i to file %s : %s" % (
                            nsave + 1, nFound, frame, frame + intvl, fout, formula))
                    traj_slice.comms = ["%s atoms %s frame %i charge %+.3f sz %+.3f sz^2 %.3f"
                                        % (formula, commadash(atoms), f, sum(self.Charges[f][atoms]),
                                           sum(self.Spins[f][atoms]), sum([j ** 2 for j in self.Spins[f][atoms]])) for f
                                        in range(frame, frame + intvl)]
                    if self.align:
                        traj_slice.center()
                        traj_slice.align()
                ...
                if event['output_id'] == iout:
                    ...
                    traj_slice.comms = ["%s atoms %s frame %i charge %+.3f sz %+.3f sz^2 %.3f"
                                        % (event['equation'], commadash(a), f, sum(self.Charges[f][a]),
                                           sum(self.Spins[f][a]), sum([j ** 2 for j in self.Spins[f][a]])) for f in
                                        range(fstart, fend + 1)]
                    if self.align:
                        traj_slice.center()
                        traj_slice.align()
    ...
    def getNeutralizing(self, frame1, frame2, atoms, tol=0.25):
        ...
        molID_sorted_maxdist = [list(Candidates.keys())[i] for i in
                                np.argsort([c.maxdx for c in list(Candidates.values())])]
        keep_molID = []
        success = False
        curr_chg = chg
        curr_spn = spn
        ...
        for molID in molID_sorted_maxdist:
            ...
            if len(keep_molID) == limit:
                ...
                break
        if self.printlvl >= 1:
            print("Neutralization %s: charge \x1b[94m%+.3f\x1b[0m -> \x1b[92m%+.3f\x1b[0m, spin %+.3f -> %+.3f" % (
                'success' if success else 'failure', chg, curr_chg, spn, curr_spn), end=' ')
            if keep_molID:
                print("added %s (molIDs %s)" % (
                    formulaSum([self.TimeSeries[m]['graph'].ef() for m in keep_molID]), ' '.join(keep_molID)))
            else:
                print()
