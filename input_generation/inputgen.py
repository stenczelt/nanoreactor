...
parser.add_argument("-c", "--coordinate_filename",
                    help="filename of the coordinates, please include the FULL PATH and file extension - must be .XYZ")
parser.add_argument("-ivr", "--inner_volume_ratio", type=float,
                    help="ratio of volume of inner sphere to sum of volumes of atoms, should be a float.  Default is 1.45")
parser.add_argument("-ovr", "--outer_volume_ratio", type=float,
                    help="ratio of volume of outer sphere to volume of inner sphere, should be a float.  Default is 3.0")
...
parser.add_argument("-bch", "--mdbc_hydrogen_input",
                    help="whether mdbc_hydrogen is yes or no in the input file. Default is yes")
parser.add_argument("-r2", "--mdr2_count_input", type=int,
                    help="the number of r2 values to iterate over for the input files. Default is 1")
parser.add_argument("-r2f", "--mdr2_factor",
                    help="the factor that determines the spread in r2 values sample. Default is 0.03 i.e. will sample (1.0, 0.97. 1.03, ...) * recommended radius")
parser.add_argument("-k2", "--mdk2_count_input", type=int,
                    help="the number of k2 values to iterate over for the input files. Default is 1, max is 20")
parser.add_argument("-ms", "--mdbc_mass_scaled_input",
                    help="whether mdbc_mass_scaled set to yes or no in input file. can be yes, no, or both (makes 2x as many files). Default is yes")
...
parser.add_argument("-s", "--submit_run", action='store_true',
                    help="whether or not to submit run.sh scripts. If the flag is included, it will submit.")
...
if args.coordinate_filename:
    if (args.coordinate_filename[-4] + args.coordinate_filename[-3] + args.coordinate_filename[-2] +
            args.coordinate_filename[-1] != ".xyz"):
    ...
    for mdk2 in mdk2list:
        for yn in ynlist:
            ...
            isg.generateinputfile(args.output_folder, filename, args.coordinate_filename, charge_input, basis_set_input,
                                  t_initial_input, dispersion_input, thermostat_input, t_zero_input, lnvtime_input,
                                  convthre_input,
                                  level_shift_input, level_shift_vala_input, level_shift_valb_input, mix_guess_input,
                                  integrator_input,
                                  run_input, method_input, scf_input, timings_input, nsteps_input, gpus_input,
                                  maxit_input,
                                  mdbc_input, mdr1, mdk1, mdr2, mdk2, mdbc_hydrogen_input, yn)

            # write to the log
            # TODO fix this --- database???
            log_file.write(filename + ', r2 = ' + str("{:6.2f}".format(mdr2)) + ', k2 = ' + str(
                "{:3.1f}".format(mdk2)) + ', mdbc_h_scaled = ' + yn + '\n')
