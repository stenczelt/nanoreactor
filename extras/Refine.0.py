...
parser.add_argument('-dt', action="store_true",
                    help='Save space by deleting large (>1MB) files from failed transition state results (stage 2)')
parser.add_argument('-ft', action="store_true",
                    help='Force transition state calculations for all folders with no data (stage 2)')
...
def chkstr(sxyz, chgspn=False):
    ...
    if not os.path.exists(vxyz):
        ...
        SMol.write(vxyz)
    T = Nanoreactor(vxyz, enhance=1.2, printlvl=-1, boring=[], delay=0, learntime=0, rxntime=10, metastability=1.0,
                    pcorrectemit=0.0)
    ...
    ostr = ""
    ostr += ', '.join(["%s (%s)" % (i['graph'].ef(), commadash(i['graph'].L())) for i in list(T.TimeSeries.values()) if
                       i['signal'][0][1]])
    ostr += " -> "
    ostr += ', '.join(["%s (%s)" % (i['graph'].ef(), commadash(i['graph'].L())) for i in list(T.TimeSeries.values()) if
                       i['signal'][-1][1]])
    ...
    satoms = itertools.chain(
        *[i['graph'].L() for i in list(T.TimeSeries.values()) if i['signal'][-1][1] and i['signal'][0][1]])
    ...
    if chgspn:
        ...
        srch = lambda s: np.array(
            [float(re.search('(?<=%s )[-+]?[0-9]*\.?[0-9]*([eEdD][-+]?[0-9]+)?' % s, c).group(0)) for c in sm.comms])
...
def get_frags(dnm, rxnm, depth, max_depth=1):
    ...
    for splxyz in splxyzs:
        ssdir = os.path.join(os.path.split(splxyz)[0],
                             re.sub("^reaction_[0-9]*(_[0-9]+)?_", "", os.path.split(splxyz)[1]).replace(".xyz",
                                                                                                         "").replace(
                                 "split", "split_"))
        ssdirs.append(ssdir)
...
def manage_an(f, dnm):
    ...
    if not os.path.exists(fresult):
        ...
        if args.qa:
            ...
            with open(os.path.join(dnm, '.runpid'), 'w') as f:
                print(os.getpid(), file=f)
            tag = fn + '___' + dnm
            queue_up_src_dest(wq,
                              "python AnalyzeReaction.py %s.xyz b3lyp 6-31g* > %s.log 2> %s.err" % (rxnm, rxnm, rxnm),
...
def manage_gs(d0, init):
        ...
        if os.path.exists(fout) and os.path.exists(arch):
            ...
            for line in open(fout):
                ...
                if "is inconsistent with the spin-z" in line:
                    ...
                    break
                if (
                        "Failed to find HF/KS stable state" in line or "Too many attempts at finding a HF-stable state" in line):
                    status = 'Failed'
            ...
            if status != 'Failed':
                if os.path.getsize(os.path.join(dgs, "gs_result.tar.bz2")) > 0:
                    ...
                    if os.path.exists(os.path.join(dgs, ".extracted")):
                        diff_time = os.stat(os.path.join(dgs, "gs_result.tar.bz2")).st_mtime - os.stat(
                            os.path.join(dgs, ".extracted")).st_mtime
                    ...
                    dcan = [i for i in os.listdir(dgs) if
                            i not in [os.path.split(lstr)[1], 'Vfile.txt', 'tsestimate.xyz']]
    ...
    if status == 'Launch':
        ...
        if args.qg:
            ...
            with open(os.path.join(dgs, '.runpid'), 'w') as f:
                print(os.getpid(), file=f)
            tag = d0 + '___' + init
            queue_up_src_dest(wq, "python growing-string.py %s initial.xyz b3lyp 6-31g* > gs.log 2> gs.err" % (
                '-stab' if do_stable else ''),
...
def manage_ts(dts, init, lstr, Launch):
    ...
    V = [float(line.strip()) for line in os.popen(
        "tac %s 2> /dev/null | awk '(p && NF==0) {exit} (NF>0) {p=1; print $3}' | tac" % os.path.join(dgs,
                                                                                                      "Vfile.txt")).readlines()]
    ...
    if os.path.exists(tslog) and os.path.exists(os.path.join(dts, 'ts_result.tar.bz2')):
        ...
        if not os.path.exists(ircxyz) and os.path.getsize(os.path.join(dts, 'ts_result.tar.bz2')) > 0:
            ...
            if os.path.exists(os.path.join(dts, ".extracted")):
                diff_time = os.stat(os.path.join(dts, "ts_result.tar.bz2")).st_mtime - os.stat(
        ...
        if any(['IRC Failure' in line for line in open(tslog).readlines()]):
            ...
            for line in open(tslog).readlines():
                ...
                if 'IRC calculation failed in forward direction, error =' in line:
                    errmsg.append(
                        'Fwd: %s ' % line.replace('IRC calculation failed in forward direction, error =', '').strip())
            ...
            print_stat(calctype, "Failure", dts, msg=("(%s)" % ', '.join(errmsg)) if len(errmsg) > 0 else '',
                       ansi="\x1b[1;91m")
        ...
        if IRC_Failure and args.dt:  # Delete some files from archives
            ...
            bigfiles = [line.strip() for line in os.popen(
                "tar -tvf %s | awk '($3>1000000) {print $NF}'" % os.path.join(dts, "ts_result.tar.bz2")).readlines()]
            if len(bigfiles) > 0:
    ...
    if TS_Success:
        ...
        if Launch:
            ...
            if max(V) > 200:
                ...
                with open(os.path.join(dts, '.runpid'), 'w') as f:
                    print(os.getpid(), file=f)
...
def manage_fs(d0, init):
    ...
    if os.path.exists(fslog):
        Exist = True
        Launch = False
        if not os.path.exists(ircxyz) and os.path.exists(os.path.join(dfs, 'fs_result.tar.bz2')) and os.path.getsize(
                os.path.join(dfs, 'fs_result.tar.bz2')) > 0:
            diff_time = 1
            if os.path.exists(os.path.join(dfs, ".extracted")):
                diff_time = os.stat(os.path.join(dfs, "fs_result.tar.bz2")).st_mtime - os.stat(
        ...
        if any([('IRC Failure' in line or 'fatal error' in line) for line in open(fslog).readlines()]):
            ...
            print_stat(calctype, "Failure", dfs, msg=("(%s)" % ', '.join(errmsg)) if len(errmsg) > 0 else '',
                       ansi="\x1b[1;91m")
        elif not os.path.exists(ircxyz):
            # ----
            # TS calculation finished but unfortunately did not provide an IRC coordinate
    ...
    if Launch:
        ...
        if args.qf:
            ...
            with open(os.path.join(dfs, '.runpid'), 'w') as f:
                print(os.getpid(), file=f)
    ...
    if Exist and not IRC_Failure:
        if chkstr.rearrange:
            print_stat(calctype, "Rearrangement", dfs, ansi="\x1b[1;93m",
                       msg="(Different molecules found)" if not chkstr.equals else "(Same molecules)")
            if chkstr.equals: FS_Finished = True
        elif chkstr.noreact:
            print_stat(calctype, "No Reaction", dfs, ansi="\x1b[1;93m",
                       msg="(Different molecules found)" if not chkstr.equals else "(Same molecules)")
            if chkstr.equals: FS_Finished = True
        elif running:
