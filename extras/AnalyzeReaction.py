...
from nanoreactor.nifty import _exec, printcool
...
def tarexit():
    ...
    _exec("tar cjf result.tar.bz2 *.xyz %s" % ' '.join(
        [i for i in os.listdir('.') if (i.startswith('qc') and not os.path.isdir(i))]), print_command=False)
...
def QCOptIC(M, frame, chg, spn, basename, check=False, gtol=1000, dxtol=5000, etol=300, cyc=50, nanoref=None):
    ...
    try:
        return QCOpt(M, frame, chg, spn, basename, cart=False, check=check, gtol=gtol, dxtol=dxtol, etol=etol, cyc=cyc,
                     nanoref=nanoref)
    except:
        try:
            traceback.print_exc()
            print("Cartesian..", end=' ')
            return QCOpt(M, frame, chg, spn, basename, cart=True, check=check, gtol=gtol, dxtol=dxtol, etol=etol,
                         cyc=cyc, nanoref=nanoref)
        except:
            traceback.print_exc()
...
print("Q-Chem optimization %s reactants and %s products" % ((preserves if set(AtomsQCR) == set(AtomsFirst) else alters),
                                                            (preserves if set(AtomsQCP) == set(AtomsLast) else alters)))
...
if NanoEqual(NanoQCR, NanoQCP):
    print(
        "Q-Chem reactant and product optimizations converged to the same point; reaction may have been \x1b[93mspontaneous\x1b[0m")
...
def InterpolatePath(Path1, fnm, verbose=True, outfnm=None):
    ...
    if verbose: print("Before velocity equalizing, movement is %.3f +- %.3f A, max %.3f, min %.3f" % (
    np.mean(ArcPath1), np.std(ArcPath1), np.max(ArcPath1), np.min(ArcPath1)))
    MaxVel = np.max(ArcPath1)
    ConstantVelocity(Path1, RMSD=True)
    ArcPath1 = arc(Path1, RMSD=True)
    ArcPath1Cumul = np.insert(np.cumsum(ArcPath1), 0, 0.0)
    if verbose: print("After velocity equalizing,  movement is %.3f +- %.3f A, max %.3f, min %.3f" % (
    np.mean(ArcPath1), np.std(ArcPath1), np.max(ArcPath1), np.min(ArcPath1)))
    ...
    if not os.path.exists(path2fnm):
        _exec("Nebterpolate.py %s %i %s" % (path1fnm, smoothing_strength, path2fnm), print_to_screen=verbose,
              print_command=verbose, persist=True)
    else:
    ...
    if os.path.exists(path2fnm):
        ...
        ArcPath2 = arc(Path2, RMSD=True)
        if verbose: print("Velocity equalized: movement is %.3f +- %.3f A, max %.3f, min %.3f" % (
        np.mean(ArcPath2), np.std(ArcPath2), np.max(ArcPath2), np.min(ArcPath2)))
...
if len(satoms) > 0:
    print("After optimizing endpoints, atoms", ', '.join(
        ["%s (%s)" % (i['graph'].ef(), commadash(i['graph'].L())) for j, i in list(NanoQCR.TimeSeries.items()) if
         j in sgid0]), "no longer participate in the reaction")
...
def GetIntermediates(QC0, NanoQC0, QC1, NanoQC1):
    ...
    for actives in activegrps:
        ...
        # print sgid0a, sgid1a
        print("reaction    :", ', '.join(
            ["%s (%s)" % (i['graph'].ef(), commadash(i['graph'].L())) for j, i in list(NanoQC0.TimeSeries.items()) if
             j not in sgid0a]), end=' ')
        print("->", ', '.join(
            ["%s (%s)" % (i['graph'].ef(), commadash(i['graph'].L())) for j, i in list(NanoQC1.TimeSeries.items()) if
             j not in sgid1a]))
        if len(sgid1a) > 0:
            print("speculators :", ', '.join(
                ["%s (%s)" % (i['graph'].ef(), commadash(i['graph'].L())) for j, i in list(NanoQC1.TimeSeries.items())
                 if j in sgid1a]))
        ...
        if chgpass:
            ...
            if ((nei - spnzi) / 2) * 2 != (nei - spnzi):
                ...
                rx0 = '+'.join([("%i" % iso0.count(i) if iso0.count(i) > 1 else "") + NanoQC0.Isomers[i].ef() for i in
                                sorted(set(iso0))])
                tx0 = str([commadash(sorted([actives.index(i) for i in NanoQC0.TimeSeries[g]['graph'].L()])) for g in
                           gid0]).replace(" ", "")
                gid1 = set(NanoQC1.TimeSeries.keys()) - set(sgid1a)
                iso1 = [NanoQC1.TimeSeries[i]['iidx'] for i in gid1]
                rx1 = '+'.join([("%i" % iso1.count(i) if iso1.count(i) > 1 else "") + NanoQC1.Isomers[i].ef() for i in
                                sorted(set(iso1))])
                tx1 = str([commadash(sorted([actives.index(i) for i in NanoQC1.TimeSeries[g]['graph'].L()])) for g in
                           gid1]).replace(" ", "")
                evector = "%s -> %s" % (rx0, rx1)
                tvector = "%s -> %s" % (tx0, tx1)
                QC0.comms = ["Reaction: formula %s atoms %s charge %+.3f sz %+.3f sz^2 %.3f" % (
                evector, tvector, Chg0[i], SpnZ0[i], Spn20[i]) for i in range(len(QC0))]
                QC1.comms = ["Reaction: formula %s atoms %s charge %+.3f sz %+.3f sz^2 %.3f" % (
                evector, tvector, Chg1[i], SpnZ1[i], Spn21[i]) for i in range(len(QC1))]
    ...
    for iframe in [int(round(i)) for i in np.linspace(0, len(M2), min(len(M2) + 1, max(11, float(len(M2)) / 10)))]:
        ...
        if NanoEqual(NanoQCI0, NanoQCR):
            ...
            if os.path.exists("qcopt_intermediates_%i.out" % iframe):
                QCI = Molecule("qcopt_intermediates_%i.out" % iframe,
                               errok=['SCF failed to converge', 'Maximum optimization cycles reached'])
            else:
                QCI = deepcopy(QCR)
                QCI_ = Molecule("qcmin_intermediates_%i.xyz" % iframe)
                QCI.xyzs[-1] = QCI_.xyzs[-1]
        ...
        if Found:
            ...
            for info in infos:
                Write = True
                if any([((NanoEqual(NanoQCL, NanoQCS[sn][0])) and (NanoEqual(NanoQCI, NanoQCS[sn][1]))) for sn in
                        range(len(NanoQCS))]):
                    print("Found a \x1b[93mduplicate pathway\x1b[0m")
                    Write = False
                elif any([((NanoEqual(NanoQCL, NanoQCS[sn][1])) and (NanoEqual(NanoQCI, NanoQCS[sn][0]))) for sn in
                          range(len(NanoQCS))]):
                    print("Found a \x1b[93mreverse pathway\x1b[0m")
                    Write = False
                elif ((any([NanoEqual(NanoQCL, NanoQCS[sn][0]) for sn in range(len(NanoQCS))]) and any(
                        [NanoEqual(NanoQCI, NanoQCS[sn][1]) for sn in range(len(NanoQCS))])) or
                      (any([NanoEqual(NanoQCL, NanoQCS[sn][1]) for sn in range(len(NanoQCS))]) and any(
                          [NanoEqual(NanoQCI, NanoQCS[sn][0]) for sn in range(len(NanoQCS))]))):
                ...
                if Write:
                    ...
                    if QCL.full:
                        ...
                        for last in LastFrames[::-1]:
                            ...
                            NanoQC0 = Nanoreactor("qcmin_intermediates_%i.xyz" % last, enhance=1.2, printlvl=-1,
                                                  boring=[])
                    ...
                    Mid.comms = ["Reaction: formula %s atoms %s charge %+.3f sz %+.3f sz^2 %.3f" % (
                    evector, tvector, chgi, spnzi, spn2i) for i in range(len(Mid))]
                    ...
                    print("Wrote segment to %s, length %i(QM) + %i(MD) + %i(QM) = %i" % (
                    splitfn, len(QC0), len(Mid), len(QC1), len(Segment)))
                    INum += 1
            NanoRef = deepcopy(NanoQCI)
            RefFrm = iframe
...
print("Trimming the original path, length %i(old) -> %i(new) = %i(QM) + %i(MD) + %i(QM)" % (
len(RxnPath), len(RxnPath2), len(QCRF), PIFrame - RFFrame - 1, len(QCPI)))
basefn = re.sub("_stage[0-9]", "", os.path.splitext(os.path.split(sys.argv[1])[-1])[0])
